...
async function executeAankoCommand() {
    ...
    const messageCount = parseInt(document.getElementById('aankoExecuteCount').value) || 10;
    ...
    progressText.textContent = `${tokens.length}トークン x ${channelIds.length}チャンネル 送信中... (各${messageCount}メッセージ, 間隔${delaySeconds}秒)`;
...
async function createAheButtonAndClick() {
    ...
    try {
        ...
        for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
            ...
            progressText.textContent = `トークン ${tokenIndex + 1}/${tokens.length}: 全チャンネルにボタン作成中...`;
            ...
            try {
                ...
                if (createResult.success) {
                    ...
                    console.log(`Token ${tokenIndex + 1}: Created ${createResult.buttonsCreated} buttons`);
                } else {
                    console.log(`Token ${tokenIndex + 1}: Button creation failed - ${createResult.error}`);
                    continue;
                }
            } catch (e) {
                if (e.name === 'AbortError') throw e;
                console.error(`Token ${tokenIndex + 1}: Button creation error - ${e.message}`);
                continue;
            }
            
            let totalClicks = 0;
            let currentChannelIndex = 0;
            let clicksInCurrentChannel = 0;
            
            const fireClick = (channelId) => {
                fetch('/api/ahe-click-single-channel', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        token,
                    ...
                    if (result.success) {
                        console.log(`Fire-and-forget click sent to ${channelId}`);
                    }
                }).catch(e => {
            ...
            while (totalClicks < clickCount && !shouldStopAheButton) {
                const currentChannel = channelIds[currentChannelIndex];
                
                progressText.textContent = `トークン ${tokenIndex + 1}/${tokens.length}: チャンネル ${currentChannelIndex + 1}/${channelIds.length} - クリック ${clicksInCurrentChannel + 1}/${clicksPerChannel} (合計: ${totalClicks + 1}/${clickCount})`;
                
                fireClick(currentChannel);
                totalClicksSent++;
                totalClicks++;
                clicksInCurrentChannel++;
                
                if (clicksInCurrentChannel >= clicksPerChannel) {
                    currentChannelIndex = (currentChannelIndex + 1) % channelIds.length;
                    clicksInCurrentChannel = 0;
                    console.log(`Moving to channel ${currentChannelIndex + 1}/${channelIds.length}`);
                }
