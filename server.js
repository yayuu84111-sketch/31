...
app.post('/api/check-channel-permissions', async (req, res) => {
    try {
        ...
        for (const channel of allChannels) {
            ...
            const canUseAppCommands = (permissions & USE_APPLICATION_COMMANDS) !== 0n;
...
app.post('/api/send-dm', async (req, res) => {
    try {
        const { tokens, channelIds, message, userIds, mentionCount, count, delayMs, includeRandomChars } = req.body;
        ...
        const messageCount = Math.max(parseInt(count) || 1, 1);
        const delayTime = Math.max(parseInt(delayMs) || 500, 10);
        ...
        // 全チャンネル×全トークンを同時並列で送信
        const sendToChannel = async (token, channelId) => {
            let channelSuccess = 0;
            let channelFailed = 0;
            ...
            for (let i = 0; i < messageCount; i++) {
                try {
                    ...
                    let content = message || `# [あんこちゃんですあんこ鯖では主にピクセルガンの無償代行や荒らしを行っていますスマホ勢荒らし知識がなくても自作あんこアプリで初日から最強荒らし勢になろうピクセルガンも初日で最強になろう](<https://nemtudo.me/e/aTPA>)
# 🤍🩷💚🖤❤️🩵🩶🧡💙💛💜🤎❣️💗💝💓❤️‍🩹❤️‍🔥💞💘💖💕
# 😉あんこちゃんだよ！
# 🩵あんこ鯖では主にピクセルガンの無償代行
# 🧡ワイワイ雑談‼️
...
`;
                    ...
                    if (selectedMentions > 0 && mentionIds.length > 0) {
                        ...
                        const toMention = shuffled.slice(0, selectedMentions);
                        const mentions = toMention.map(id => `<@${id}>`).join(' ');
                    ...
                    channelFailed++;
                    console.log(`Failed to send message to ${channelId}:`, msgError.response?.data?.message || msgError.message);
                    
                    if (msgError.response?.status === 429) {
                        const retryAfter = msgError.response?.data?.retry_after || 1;
                        await delay(retryAfter * 1000);
                    }
                }
        ...
        for (const token of tokens) {
            for (const channelId of channelIds) {
                allPromises.push(sendToChannel(token, channelId));
            }
        }
...
app.post('/api/ahe-instant-parallel-button', async (req, res) => {
    try {
        const { token, channelIds, guildId, message, userIds, mentionCount, includeEveryone: reqIncludeEveryone, randLen, clickCount: reqClickCount } = req.body;
        ...
        for (let i = 0; i < channels.length; i++) {
            ...
            console.log(`[AHE-SEQUENTIAL] Processing channel ${i + 1}/${channels.length}: ${channelId}`);
            
            // Step 1: Send the command to create button (with retry for rate limits)
            const sendResult = await createButtonForChannel(channelId);
            
            if (!sendResult.sent) {
                console.log(`[AHE-SEQUENTIAL] FAILED: Command failed for channel ${channelId}: ${sendResult.error}`);
                failedChannels.push({ channelId, error: sendResult.error });
            ...
            console.log(`[AHE-SEQUENTIAL] Command sent for channel ${channelId}, waiting for confirmation...`);
            ...
            if (result.success) {
                ...
                console.log(`[AHE-SEQUENTIAL] OK! Button confirmed for channel ${channelId} (${buttonData.length}/${channels.length})`);
            } else {
                console.log(`[AHE-SEQUENTIAL] FAILED: Could not confirm button for channel ${channelId}`);
        ...
        // Phase 2: Click all buttons multiple times
        console.log(`[AHE-SEQUENTIAL] Phase 2: Clicking each button ${clickCount} times...`);
        let clicksSent = 0;
        
        for (let clickRound = 0; clickRound < clickCount; clickRound++) {
            console.log(`[AHE-SEQUENTIAL] Click round ${clickRound + 1}/${clickCount}`);
            ...
            for (const data of buttonData) {
                ...
                try {
                    ...
                    console.log(`[AHE-SEQUENTIAL] Click ${clickRound + 1} sent to ${data.channelId}`);
                    clicksSent++;
                } catch (e) {
                    const errorMessage = e.response?.data?.message || e.message;
                    console.log(`[AHE-SEQUENTIAL] Click ${clickRound + 1} failed for ${data.channelId}: ${errorMessage}`);
            ...
            // Delay between click rounds to avoid rate limiting
            if (clickRound < clickCount - 1) {
                await delay(300);
            }
        }

        console.log(`[AHE-SEQUENTIAL] Phase 2 complete: Fired ${clicksSent} clicks (target: ${buttonData.length * clickCount})`);
        ...
        res.json({
            ...
            clicksSent: clicksSent,
            targetClicks: buttonData.length * clickCount,
            skippedChannels,
